...
namespace RushBlog.WebAdmin
{
	public class Startup
	{
		...
		public void Configure(IApplicationBuilder app, IHostingEnvironment env)
		{
			...
			var children = staticFiles.GetChildren();
			var kvp = children.Select(item => new KeyValuePair<string, string>(item.Key, item.Value));
			var d = kvp.ToDictionary(x => x.Key, x => x.Value);

			foreach(var item in d)
			{
				app.UseStaticFiles(new StaticFileOptions()
				{
					FileProvider = new PhysicalFileProvider(item.Value),
